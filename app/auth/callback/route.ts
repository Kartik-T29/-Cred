  ...
  if (code) {
    ...
    try {
      ...
      if (!error) {
        ...
        if (user && (!user.user_metadata || !user.user_metadata.profile_completed)) {
          // Redirect new users to complete their profile if the route exists
          // For now, we'll check if the route exists before redirecting
